...
export const TopListCard: React.FC<TopListCardProps> = ({
  ...
  return (
    ...
    >
      <div className="text-[11.5px] font-medium text-fg-2 group-hover:text-white/80 transition-colors">{label}</div>

      {items.length === 0 ? (
        <div className="mt-2 text-[11px] text-fg-3 group-hover:text-white/70 transition-colors">{emptyText}</div>
      ) : (
        <ol className="mt-1.5 space-y-[3px]">
          {items.map((item, i) => (
            <li key={item.id} className="relative flex items-center gap-1.5 h-[17px] px-1 -mx-1 rounded-[4px] overflow-hidden">
              <span
                aria-hidden
                className="absolute inset-y-0 left-0 bg-accent-weak group-hover:bg-white/20 rounded-[4px] transition-colors"
                style={{ width: `${Math.max((item.weight / max) * 100, 4)}%` }}
              />
              <span className="relative z-10 text-[10px] font-semibold text-fg-3 group-hover:text-white/70 transition-colors tnum w-[9px] shrink-0">{i + 1}</span>
              <span className="relative z-10 text-[11px] font-medium text-fg group-hover:text-white transition-colors truncate flex-1" title={item.label}>
                {item.label}
              </span>
              <span className="relative z-10 text-[11px] font-semibold text-fg-2 group-hover:text-white transition-colors tnum shrink-0">{item.value}</span>
            </li>
          ))}
        </ol>
      )}

      {sub && <div className="text-[10.5px] mt-1.5 text-fg-3 group-hover:text-white/80 transition-colors tnum">{sub}</div>}
    </div>
  );
};
